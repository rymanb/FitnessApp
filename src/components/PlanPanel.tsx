...
import { View, Pressable, TextInput, ScrollView, Alert, ActivityIndicator, Share } from "react-native";
...
export default function PlanPanel({isVisible, onClose, existingPlan}: PlanPanelProps) {
    ...
    // A muscle promoted to primary in any exercise is removed from secondary.
    ...
    const handleSharePlan = async () => {
        ...
        try {
            ...
            const response = await apiFetch(`${BACKEND_URL}/api/v1/plans/${editingPlanId}/share`, {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${token}` },
            });
            if (!response.ok) throw new Error('Share request failed');
            const { shareId } = await response.json();
            const link = `${BACKEND_URL}/share/${shareId}`;
            await Share.share({ message: `Check out my workout plan "${draftName}"!\n\n${link}` });
        } catch {
            Alert.alert('Share Failed', 'Could not create a share link. Please try again.');
        } finally {
            setIsSharing(false);
        }
    };
                    ...
                    {editingPlanId && (
                        <View className="flex-row gap-2">
                            <Pressable
                                ...
                                disabled={isSharing}
                                className="bg-blue-500/10 p-4 rounded-xl active:bg-blue-500/20 mr-2 border border-blue-500/30"
                            >
                                {isSharing ? (
